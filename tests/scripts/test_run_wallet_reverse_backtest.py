...
def test_run_wallet_reverse_backtest_filters_fills_by_date_window_exclusive_end(
    ...
    mocker.patch.object(runner, "read_wallets", return_value=_wallets(["0xaaa"]))
    mocker.patch.object(runner, "read_fills", return_value=fills)
    ...
    mocker.patch.object(runner, "_backtest_wallet", side_effect=fake_backtest_wallet)

    train_result = runner.run_wallet_reverse_backtest(
    ...
    oos_result = runner.run_wallet_reverse_backtest(
        runner.WalletReverseBacktestConfig(
...
def test_wallet_reverse_date_end_filter_description_documents_exclusive_end():
    config = runner.WalletReverseBacktestConfig(
        report=None,
        date_start=pd.Timestamp("2026-01-10T00:00:00Z"),
        date_end=pd.Timestamp("2026-01-20T00:00:00Z"),
    )

    assert runner._date_end_filter(config) == "fill_time < 2026-01-20"
