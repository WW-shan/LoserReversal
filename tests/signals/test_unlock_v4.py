...
def _prices(start: str = "2026-01-01", periods: int = 60) -> pd.Series:
...
def test_single_event_emits_entry_three_days_before_unlock_and_exit_on_unlock():
    prices = {"ARB": _prices()}
    events = _events([{"token": "ARB", "unlock_date": pd.Timestamp("2026-01-15T00:00:00Z")}])

    result = unlock_short_72h(events, prices)
...
def test_event_below_min_unlock_pct_omits_token():
    ...
    assert unlock_short_72h(events, prices) == {}
...
def test_can_include_token_without_hyperliquid_perp_when_not_required():
    ...
    result = unlock_short_72h(events, prices, require_hl_perp=False)
...
def test_two_spaced_events_emit_two_non_overlapping_pairs():
    ...
    events = _events(
        [
            ...
            {"token": "ARB", "unlock_date": pd.Timestamp("2026-01-22T00:00:00Z")},
        ]
    )

    entries, exits = unlock_short_72h(events, prices)["ARB"]

    assert bool(entries.loc["2026-01-12"])
    assert bool(exits.loc["2026-01-15"])
    assert bool(entries.loc["2026-01-19"])
    assert bool(exits.loc["2026-01-22"])
...
def test_overlapping_second_event_is_skipped():
    ...
    events = _events(
        [
            {"token": "ARB", "unlock_date": pd.Timestamp("2026-01-15T00:00:00Z")},
            {"token": "ARB", "unlock_date": pd.Timestamp("2026-01-17T00:00:00Z")},
        ]
    )
...
def test_multiple_tokens_returns_only_qualifying_tokens():
    ...
    assert set(unlock_short_72h(events, prices)) == {"ARB"}
...
def test_naive_price_index_matches_utc_price_index_without_mutating_caller():
    ...
    aware_entries, aware_exits = unlock_short_72h(events, {"ARB": aware_prices})["ARB"]
    naive_entries, naive_exits = unlock_short_72h(events, {"ARB": naive_prices})["ARB"]
...
def test_pre_window_days_must_be_positive():
    ...
    for pre_window_days in [0, -1]:
        with pytest.raises(ValueError):
            unlock_short_72h(events, prices, pre_window_days=pre_window_days)
    ...
    coverage = pd.DataFrame(
        [
            {"token": "ARB", "unlock_date": "2026-01-15", "coverage_status": "ok"},
            {
                ...
                "coverage_status": "no_candle_file",
            },
        ]
    )

    result = unlock_short_72h(events, prices, coverage=coverage)
