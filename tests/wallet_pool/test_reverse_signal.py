...
def test_score_wallet_fills_does_not_infer_settle_interval_from_one_prior_funding_row() -> None:
    fills = _fills_frame([_fill(time="2026-05-26T15:45:00Z")])
    funding = pd.DataFrame(
        {"funding_zscore": [0.0]},
        index=pd.to_datetime(["2026-05-26T08:00:00Z"], utc=True),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"funding_settle_minutes_before": 0}, "funding_settle_minutes_before"),
        ({"funding_settle_minutes_before": -1}, "funding_settle_minutes_before"),
        ({"funding_settle_minutes_before": 30.5}, "funding_settle_minutes_before"),
